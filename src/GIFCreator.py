...
class GIFCreator:
    ...
    def __createFrames(self, mesh_handler):
        ...
        axes.add_collection3d(
            mplot3d.art3d.Poly3DCollection(vectors, color=self.model_color)
        )
        axes.add_collection3d(
            mplot3d.art3d.Line3DCollection(
                vectors, color=self.edge_color, linewidth=self.line_width
            )
        )
        ...
        for i in range(self.frames):
            # Rotate the view
            axes.view_init(
                elev=self.elevation, azim=self.init_angle + 360 / self.frames * i
            )
